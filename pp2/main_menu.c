/**
* @file main_menu.c
 * @brief Obsługa głównego menu gry, ekranów trybu gry, ustawień, autorów, wyników oraz zakończenia.
 *
 * Zawiera funkcje związane z obsługą interfejsu użytkownika i ekranów pomocniczych gry
 * wykorzystujące bibliotekę Allegro 5.
 */
...
/**
 * @brief Ilość żyć ustawiona przez użytkownika.
 */
int ilosc_zyc = 1;

/**
 * @brief Wskaźnik na aktualnie odtwarzaną muzykę.
 */
ALLEGRO_SAMPLE *aktualna_muzyka = NULL;

/**
 * @brief Instancja dźwięku odtwarzanego w tle.
 */
ALLEGRO_SAMPLE_INSTANCE *muzyka_instance = NULL;

/**
 * @brief Wyświetla ekran tabeli wyników w trybie klasycznym i nieskończonym.
 *
 * @param queue Kolejka zdarzeń Allegro.
 * @param font Czcionka używana do rysowania tekstu.
 * @param sound Dźwięk odtwarzany po kliknięciu.
 */
void tabela_wynikow(ALLEGRO_EVENT_QUEUE *queue, ALLEGRO_FONT *font, ALLEGRO_SAMPLE *sound) {
    ...
    while (true) {
        ...
        draw_button_color(cofnij, font, al_map_rgb(80, 0, 0), al_map_rgb(255, 100, 100), al_map_rgb(160, 0, 0), al_map_rgb(100, 0, 0), al_map_rgb(255, 255, 255));
        al_flip_display();
...
/**
 * @brief Wyświetla ekran autorów projektu z grafikami i podpisami.
 *
 * @param queue Kolejka zdarzeń Allegro.
 * @param font Czcionka używana do tekstów.
 * @param sound Dźwięk przy klikaniu przycisku cofania.
 */
...
/**
 * @brief Wyświetla ekran wyboru trybu gry: klasyczny, nieskończony lub cofnięcie.
 *
 * @param queue Kolejka zdarzeń Allegro.
 * @param font Czcionka do rysowania tekstu.
 * @param sound Dźwięk przy zatwierdzeniu.
 * @param cancel Dźwięk przy anulowaniu.
 * @return 0 dla trybu klasycznego, 1 dla nieskończonego, -1 dla cofnięcia.
 */
...
/**
 * @brief Zwraca liczbę żyć wybraną w ustawieniach.
 *
 * @return Liczba żyć (domyślnie 1).
 */
...
/**
 * @brief Ekran ustawień gry, pozwalający zmienić liczbę żyć oraz utwór muzyczny.
 *
 * @param queue Kolejka zdarzeń Allegro.
 * @param font Czcionka używana do rysowania interfejsu.
 * @param sound Dźwięk przy zatwierdzeniu wyboru.
 * @param cancel Dźwięk przy anulowaniu lub zmniejszeniu ilości żyć.
 */
void ustawienia(ALLEGRO_EVENT_QUEUE *queue, ALLEGRO_FONT *font, ALLEGRO_SAMPLE *sound, ALLEGRO_SAMPLE *cancel) {
    ...
    snprintf(zycia[2].label, sizeof(zycia[2].label), "%s", "+");
    for (int i = 0; i < 3; i++) {
    ...
    while (true) {
        ...
        snprintf(zycia[1].label, sizeof(zycia[1].label), "%d", ilosc_zyc);
        al_draw_text(font, al_map_rgb(255, 255, 255),
        ...
        if (ev.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
            ...
            if (is_button_clicked(zycia[2], mx, my)) {
                al_play_sample(sound, 1.5, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
                ilosc_zyc++;
            }
        } else if (ev.type == ALLEGRO_EVENT_KEY_DOWN && ev.keyboard.keycode == ALLEGRO_KEY_ESCAPE) {
...
/**
 * @brief Główne menu gry z opcjami: Start, Tabela wyników, Ustawienia, Autorzy, Wyjdź.
 *
 * @param display Wskaźnik na ekran Allegro.
 * @param queue Kolejka zdarzeń Allegro.
 * @param font Czcionka do rysowania tekstu.
 * @return 0 jeśli wybrano tryb klasyczny, 1 dla nieskończonego, 2 jeśli gracz opuścił grę.
 */
int main_menu(ALLEGRO_DISPLAY *display, ALLEGRO_EVENT_QUEUE *queue, ALLEGRO_FONT *font) {
    ...
    ALLEGRO_BITMAP* tlo = al_load_bitmap("tlo3.png");
...
/**
 * @brief Ekran końcowy po zakończeniu gry (przegrana/wygrana).
 *
 * @param queue Kolejka zdarzeń Allegro.
 * @param font Czcionka do wyświetlenia informacji.
 * @param runda Numer rundy, do której doszedł gracz.
...
void to_jest_koniec(ALLEGRO_EVENT_QUEUE *queue, ALLEGRO_FONT *font,int runda, int tryb) {
    ALLEGRO_SAMPLE *victory = al_load_sample("victory.mp3"),
    *lose = al_load_sample("lose.mp3");
    ...
    if (tryb == 0) {
        if (runda - 1 == 13) {
            al_play_sample(victory, 1.5, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
        }
        else {
            al_play_sample(lose, 1.5, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
        }
        snprintf(tekst, sizeof(tekst), "Wygrałeś %d złotych",wygrane[runda - 1]);
    }
    else {
        snprintf(tekst, sizeof(tekst), "Doszedłeś do %d pytania",runda);
    }
    while (1) {
        draw_button_color(wyjdz, font, al_map_rgb(80, 0, 0), al_map_rgb(255, 100, 100), al_map_rgb(160, 0, 0), al_map_rgb(100, 0, 0), al_map_rgb(255, 255, 255));
        al_draw_text(font, al_map_rgb(255,255,255),
...
/**
 * @brief Wyświetla ekran umożliwiający użytkownikowi wpisanie imienia z klawiatury.
 *
 * @param queue Kolejka zdarzeń Allegro.
 * @param font Czcionka do rysowania tekstu.
 * @param name_buffer Bufor na wpisane imię (modyfikowany w miejscu).
 */
